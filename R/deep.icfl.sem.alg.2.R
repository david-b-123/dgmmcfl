deepgmm_icfl_sem.alg.2 <- function(y, numobs, p, r, k, A.list, D.list,
  ...
  ################
  l <- 1
  z2.one <- array(0, c(numobs, r[l + 1], k[l], k[l + 1]))
  
  
  l <- 2
  z.two <- array(0, c(numobs, r[l + 1], k[l]))
  ################
    ...
    for (p1 in 1 : k[l]) {
      for (p2 in 1 : k[l + 1]) {
        ...
        b <- c(ginv(H.list[[l + 1]][,,p2 ] %*% t(H.list[[l + 1]][,,p2 ]) +
                      psi.list[[l + 1]][,,p2 ])  %*% matrix(mu.list[[l + 1]][, p2], r[l + 1], numobs)) + (t(A.list[[l]])%*%(ginv(D.list[[l]][,,p1])) %*% (t(yy)))
