deepgmm_hmcfl_sem.alg.3 <- function(y, numobs, p, r, k, A.list, D.list,
...
while ((hh < it) & (ratio > eps )) {
  ...
  for (p1 in 1 : k[l]) {
    for (p2 in 1 : k[l + 1]) {
      for (p3 in 1 : k[l + 2]) {
        sigma.tilde.inv <- ginv(array(A.list[[l+1]][,,p2],dim=c(r[l+1],r[l+2])) %*% (
          array(A.list[[l+2]][,,p3],dim=c(r[l+2],r[l+3])) %*% array(omega.list[[l+2]][,,p3],dim=c(r[l+3],r[l+3]))  %*% t(array(A.list[[l+2]][,,p3],dim=c(r[l+2],r[l+3]))) + array(D.list[[l+2]][,,p3],dim=c(r[l+2],r[l+2]))
        ) %*% t(array(A.list[[l+1]][,,p2],dim=c(r[l+1],r[l+2]))))
  ...
  for (p1 in 1:k[l]) {
   ...
   zz[,,, p1]  <- aperm(array(chsi, c(r[l + 1], r[l + 1], numobs)) +
                    roy.quadro, c(3, 1, 2))
   z.one[,, p1] <- rmvnorm(numobs, rep(0, r[l + 1]), chsi) + t(roy)
   z[,, p1]  <- t(roy)
  }
  ...
  # print(c(hh,lik,adjustedRandIndex(as.vector(labels),as.vector(s[,column]))))
...
# print(c(hh,bic,adjustedRandIndex(as.vector(labels),as.vector(s[,column]))))
