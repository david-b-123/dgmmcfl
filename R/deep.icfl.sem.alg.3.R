deepgmm_icfl_sem.alg.3 <- function(y, numobs, p, r, k, A.list, D.list,
...
while ((hh < it) & (ratio > eps )) {
  ...
  # load("/home/davb/Documents/Samsung_T5/519_11_21_Deep_Statistical_Models/Deep_Gaussian_Mixture_Model/scripts/deepgmm-branch_3_0_0/r_backup/backup_v1.RData")
  p <- ncol(Y)
  if (is.null(p))
    p <- 1
  n <- nrow(Y)
  ...
  for (p1 in 1 : k[l]) {
    for (p2 in 1:k[l+1])  {
      ...
      b <- ginv(H.list[[l + 1]][,, p2] %*% t(H.list[[l + 1]][,, p2]) +
        ...
        t(H.list[[l]][,, p1]) %*% ginv(psi.list[[l]][,, p1]) %*%
