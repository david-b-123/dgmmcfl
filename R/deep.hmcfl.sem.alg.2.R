deepgmm_hmcfl_sem.alg.2 <- function(y, numobs, p, r, k, A.list, D.list,
  ...
  while ((hh < it) & (ratio > eps )) {
    ...
    for (p1 in 1 : k[l]) {
      for (p2 in 1 : k[l + 1]) {
        ...
        A <- ginv((A.list[[l + 1]][,,p2] %*% as.matrix(array(omega.list[[ l + 1 ]],dim=c(r[l+2],r[l+2],k[l+1]))[,,p2]) %*% t(A.list[[l + 1]][,,p2])+D.list[[l+1]][,,p2])) +
          t(A.list[[l]][,,1])%*%ginv(D.list[[l]][,,p1])%*%A.list[[l]][,,1]
        
        # b contains part of p_{sl} (z^{l-1})
        # =  Sigma_{sl}^(-1) mu_{sl}
        b <- c(ginv((A.list[[l + 1]][,,p2] %*% as.matrix(array(omega.list[[ l + 1 ]],dim=c(r[l+2],r[l+2],k[l+1]))[,,p2]) %*% t(A.list[[l + 1]][,,p2])+D.list[[l+1]][,,p2])) %*% (c(A.list[[l + 1]][,,p2]%*%as.matrix(array(xi.list[[l+1]],dim=c(r[l+2],k[l+1]))[,p2]))))+ (t(A.list[[l]][,,1])%*%(ginv(D.list[[l]][,,p1])) %*% (t(yy)))
        ...
        if (!is.positive.definite(chsi) || !isSymmetric(chsi)) {
    ...
    omega<-array(apply(omega.list[[l]],3,function(X){
      X<-make.positive.definite(X)
      X<-makeSymm(X)
      return(X)
    }),dim=c(r[l+1],r[l+1],k[l]))
    ...
    for (i in 1:k[l]) {
      ...
      inv_Di <- diag(1 / diag(as.matrix(D[,,i])))
      inv_Di_A <- sweep(as.matrix(AA[,,1]), 1, diag(1/as.matrix(D[,,i])), '*')
      # gamma = (A Omega_i A^T + D) ^{-1} A Omega_i
      # using Woodbury Identity
      gamma <- (inv_Di - inv_Di_A %*% 
                  chol.inv(chol.inv(omega[,, i]) +  t(as.matrix(AA[,,1])) %*% inv_Di_A) %*%
                  t(inv_Di_A)) %*% as.matrix(AA[,,1]) %*% omega[,, i]
      ...
      # y_j - A xi_i
      Y_Axi_i <- sweep(Y, 2, as.matrix(AA[,,1]) %*% xi_i, '-')
      ...
      # 
      omega[,, i] <- (diag(r[l+1]) - t(gamma) %*% as.matrix(AA[,,1])) %*% omega[,, i] +
      ...
      beta<- ginv(as.matrix(AA[,,1]) %*% omega[,,i] %*% t(as.matrix(AA[,,1])) + as.matrix(D[,,i]))%*%as.matrix(D[,,i])
      
      D1 =  diag(colSums(tau[, i]%*%t(diag(as.matrix(D[,,i])%*%(diag(p)-beta))))) / ti
    ...
    # out <- compute.est(k=k[l], p=r[l], qq=r[l+1], ps.y = ps.y.list[[l]],y =  yy,Ez = aperm(z, c(3, 1, 2)),Ezz =  aperm(zz, c(4, 2, 3, 1)))
    # 
    # A.list[[l]] <- out$A
    # D.list[[l]] <- out$D
    # w.list[[l]] <- out$w
    ...
    for (i in 1:k[l]) {
      ...
      inv_Di <- diag(1 / diag(as.matrix(D[,,i])))
      inv_Di_A <- sweep(array(AA[,,i],dim=c(r[l],r[l+1])), 1, diag(1/as.matrix(D[,,i])), '*')
      # gamma = (A Omega_i A^T + D) ^{-1} A Omega_i
      # using Woodbury Identity
      gamma <- (inv_Di - inv_Di_A %*% 
                  chol.inv(chol.inv(omega[,, i]) +  t(array(AA[,,i],dim=c(r[l],r[l+1]))) %*% inv_Di_A) %*%
                  t(inv_Di_A)) %*% array(AA[,,i],dim=c(r[l],r[l+1])) %*% omega[,, i]
      ...
      # y_j - A xi_i
      Y_Axi_i <- sweep(Y, 2, array(AA[,,i],dim=c(r[l],r[l+1])) %*% xi_i, '-')
      ...
      # 
      omega[,, i] <- (diag(r[l+1]) - t(gamma) %*% array(AA[,,i],dim=c(r[l],r[l+1]))) %*% omega[,, i] +
        t(gamma) %*% t(Y_Axi_i) %*% tY_Axi_i %*% gamma / ti -
        (xi_i - xi[,i]) %*% t(xi_i - xi[, i])
      # tau_ij yj xi_i^T +   tau_ij yj (yj - A xi)^T gamma
      A1 <- colSums(tY) %*% t(xi_i) + t(Y) %*% tY_Axi_i %*% gamma
      #
      A2 <- makeSymm(make.positive.definite((omega[,, i] + xi[, i] %*% t(xi[, i])) * ti))
      ...
      beta<- ginv((makeSymm(array(AA[,,i],dim=c(r[l],r[l+1])) %*% as.matrix(omega[,,i]) %*% t(array(AA[,,i],dim=c(r[l],r[l+1]))) + as.matrix(D[,,i]))))%*%as.matrix(D[,,i])
  ...
  for (j in 1 : layers) {
    if (j==1){
      h <- h + (k[j] - 1) + r[j] * k[j] + (r[j]*r[j+1]) - r[j+1]^2  + (r[j+1] * k[j] + (r[j+1]*r[j+1]+r[j+1])*k[j]/2)
    } else {
      h <- h + (k[j] - 1) + r[j] * k[j] + k[j] * (r[j]*r[j+1] - r[j+1]^2 )  + (r[j+1] * k[j] + (r[j+1]*r[j+1]+r[j+1])*k[j]/2)
    }
  }
  ...
  # for (j in 1 : layers) {
  #   if (j==1){
  #     h <- h + (k[j] - 1) + r[j] * k[j] + (r[j]*r[j+1]) - r[j+1]^2 
  #   } else {
  #     h <- h + (k[j] - 1) + r[j] * k[j] + k[j] * (r[j]*r[j+1] - r[j+1]^2 )  
  #   }
  # }
  # j=layers
  # h = h + (r[j+1] * k[j] + (r[j+1]*r[j+1]+r[j+1])*k[j]/2)
