deepgmm_cfl_sem.alg.2 <- function(y, numobs, p, r, k, A.list, D.list,
  ...
  #################################
  out <- deepgmm_cfl_compute.lik(y, numobs, k, xi.list, A.list, D.list, omega.list, w.list)
  py <- out$py
  ps.y <- out$ps.y
  ...
  while ((hh < it) & (ratio > eps )) {
    ...
    for (p1 in 1 : k[l]) {
      for (p2 in 1 : k[l + 1]) {
        ...
        A <- ginv((A.list[[l + 1]] %*% as.matrix(array(omega.list[[ l + 1 ]],dim=c(r[l+2],r[l+2],k[l+1]))[,,p2]) %*% t(A.list[[l + 1]])+D.list[[l+1]][,,p2])) +
          t(A.list[[l]])%*%ginv(D.list[[l]][,,p1])%*%A.list[[l]]

        # b contains part of p_{sl} (z^{l-1})
        # =  Sigma_{sl}^(-1) mu_{sl}
        b <- c(ginv((A.list[[l + 1]] %*% as.matrix(array(omega.list[[ l + 1 ]],dim=c(r[l+2],r[l+2],k[l+1]))[,,p2]) %*% t(A.list[[l + 1]])+D.list[[l+1]][,,p2])) %*% (c(A.list[[l + 1]]%*%as.matrix(array(xi.list[[l+1]],dim=c(r[l+2],k[l+1]))[,p2]))))+ (t(A.list[[l]])%*%(ginv(D.list[[l]][,,p1])) %*% (t(yy)))
    ...
    for (p1 in 1 : k[l]) {
      ...
      # b contains part of p_{sl} (z^{l-1})
      # =  Sigma_{sl}^(-1) mu_{sl}
      b <- c((ginv(array(omega.list[[ l]],dim=c(r[l+1],r[l+1],k[l]))[,,p1]))%*% (c(array(xi.list[[l]],dim=c(r[l+1],k[l]))[,p1])))+ (t(A.list[[l]])%*%(ginv(D.list[[l]][,,p1])) %*% (t(yy)))
    ...
    out <- deepgmm_cfl_compute.lik(y = y, numobs = numobs, k = k, xi.list = xi.list,A.list =  A.list, D.list = D.list,omega.list =  omega.list, w.list = w.list)
    py <- out$py
    ps.y <- out$ps.y
    ps.y.list <- out$ps.y.list
    k.comb <- out$k.comb
